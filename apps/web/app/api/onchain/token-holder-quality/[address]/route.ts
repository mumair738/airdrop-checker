import { NextRequest, NextResponse } from 'next/server';
import { isValidAddress } from '@airdrop-finder/shared';
import { goldrushClient } from '@/lib/goldrush/client';
import { SUPPORTED_CHAINS } from '@airdrop-finder/shared';
import { cache } from '@airdrop-finder/shared';

export const dynamic = 'force-dynamic';

/**
 * GET /api/onchain/token-holder-quality/[address]
 * Assess holder quality score
 * Measures holder value and engagement
 */
export async function GET(
  request: NextRequest,
  { params }: { params: Promise<{ address: string }> }
) {
  try {
    const { address } = await params;
    const searchParams = request.nextUrl.searchParams;
    const chainId = searchParams.get('chainId');

    if (!isValidAddress(address)) {
      return NextResponse.json(
        { error: 'Invalid Ethereum address' },
        { status: 400 }
      );
    }

    const normalizedAddress = address.toLowerCase();
    const cacheKey = `onchain-holder-quality:${normalizedAddress}:${chainId || 'all'}`;
    const cachedResult = cache.get(cacheKey);

    if (cachedResult) {
      return NextResponse.json({
        ...cachedResult,
        cached: true,
      });
    }

    const targetChainId = chainId ? parseInt(chainId) : 1;

    const quality: any = {
      tokenAddress: normalizedAddress,
      chainId: targetChainId,
      qualityScore: 0,
      factors: {
        retention: 0,
        diversity: 0,
        engagement: 0,
      },
      timestamp: Date.now(),
    };

    try {
      const response = await goldrushClient.get(
        `/v2/${targetChainId}/tokens/${normalizedAddress}/token_holders/`,
        { 'quote-currency': 'USD', 'page-size': 100 }
      );

      if (response.data?.items) {
        const holders = response.data.items;
        const longTerm = holders.filter((h: any) => {
          const lastTransfer = new Date(h.last_transferred_at || 0);
          const daysAgo = (Date.now() - lastTransfer.getTime()) / (1000 * 60 * 60 * 24);
          return daysAgo > 60;
        });

        quality.factors.retention = holders.length > 0 ? 
          (longTerm.length / holders.length) * 100 : 0;
        quality.factors.diversity = Math.min(100, holders.length / 10);
        quality.qualityScore = (quality.factors.retention + quality.factors.diversity) / 2;
      }
    } catch (error) {
      console.error('Error assessing quality:', error);
    }

    cache.set(cacheKey, quality, 5 * 60 * 1000);

    return NextResponse.json(quality);
  } catch (error) {
    console.error('Holder quality error:', error);
    return NextResponse.json(
      {
        error: 'Failed to assess holder quality',
        details: error instanceof Error ? error.message : 'Unknown error',
      },
      { status: 500 }
    );
  }
}






