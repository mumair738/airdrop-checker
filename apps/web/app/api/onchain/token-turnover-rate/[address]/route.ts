...
/**
 * GET /api/onchain/token-turnover-rate/[address]
 * Calculate token turnover rate
 */
export async function GET(
  ...
  try {
    ...
    const cacheKey = `onchain-turnover:${normalizedAddress}:${chainId || 'all'}`;
    ...
    try {
      ...
      if (response.data) {
        ...
        turnover.turnoverRate = marketCap > 0 ? (volume24h / marketCap) * 100 : 0;
      }
    } catch (error) {
