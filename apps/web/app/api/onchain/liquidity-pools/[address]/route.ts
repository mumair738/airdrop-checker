...
/**
 * GET /api/onchain/liquidity-pools/[address]
 * Track liquidity pool positions for a wallet
 * Uses GoldRush API for LP token data
 */
export async function GET(
  ...
  try {
    ...
    const cacheKey = `onchain-liquidity-pools:${normalizedAddress}:${chainId || 'all'}`;
    ...
    for (const chain of targetChains) {
      try {
        ...
        if (response.data?.items) {
          ...
          items.forEach((token: any) => {
            ...
            const isLP = token.contract_ticker_symbol?.includes('LP') ||
                        ...
                        token.contract_ticker_symbol?.includes('SLP') ||
            ...
            if (isLP && token.quote > 0) {
              const position = {
                ...
                lpTokenSymbol: token.contract_ticker_symbol,
        ...
        console.error(`Error fetching LP positions on ${chain.name}:`, error);
      }
    }
    ...
    console.error('On-chain liquidity pools API error:', error);
    return NextResponse.json(
      {
