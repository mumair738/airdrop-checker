...
/**
 * GET /api/onchain/token-token-holder-segmentation/[address]
 * Segment holders by balance and activity levels
 */
  ...
  try {
    ...
    try {
      ...
      if (response.data) {
        ...
        segmentation.segments.dolphins = Math.floor(holderCount * 0.09);
    ...
    console.error('Token holder segmentation error:', error);
    return NextResponse.json(
      {
